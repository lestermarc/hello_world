--------------------------------------------------------
--  DDL for Package Body ACT_LIB_PAYMENT_ISO_1_2_3
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "ACT_LIB_PAYMENT_ISO_1_2_3" 
/**
 * Génération du document Xml pour un paiment selon ISO 20022.
 * Spécialisation Deutschland
 *
 * @date 03.2012
 * @author rhermann
 * @author spfister
...
begin
  ...
  select
    XMLElement("Document",
      XMLAttributes(
        'urn:iso:std:iso:20022:tech:xsd:pain.001.002.03' as "xmlns",
      ...
      XMLElement("CstmrCdtTrfInitn",
        act_lib_payment_iso_1_2_3.group_header(
          act_mgt_payment_iso.count_creditors,
          act_mgt_payment_iso.total_amount_creditors),
        act_lib_payment_iso_1_2_3.payment_info(id_execuction)
      )
    ) into lx_data
  from DUAL;
...
begin
  select
    XMLAgg(XMLElement("PmtInf",
        ...
        case
          when (act_typ_payment_iso.gtDebtor.bank_country <> 'CH') then
            XMLElement("PstlAdr",
              ...
              XMLForest(
                act_typ_payment_iso.gtDebtor.company_address as "AdrLine",
                act_typ_payment_iso.gtDebtor.company_zip||' '||act_typ_payment_iso.gtDebtor.company_city as "AdrLine"
              )
            )
        end
      ),
      ...
      act_lib_payment_iso_1_2_3.transactions(T.CURRENCY, T.PAYMENT_TYPE)
    )) into lx_data
  from (
