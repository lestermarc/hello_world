--------------------------------------------------------
--  DDL for Package Body DOC_DOCUMENT_REVALUATION_FCT
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "DOC_DOCUMENT_REVALUATION_FCT" 
is
  /**
  ...
  */
  ...
  begin
    ...
    if (    aDRE_DATE_FROM is not null
      ...
      vWhereClause  :=
              vWhereClause || ' DMT.DMT_DATE_DOCUMENT >= to_date(''' || aDRE_DATE_FROM || ''') and DMT.DMT_DATE_DOCUMENT <= to_date(''' || aDRE_DATE_TO || ''')';
    ...
    -- Ne pas présenter les documents liés à une tranche virtuelle du risque de change
    if (vWhereClause is not null) then
      vWhereClause  := vWhereClause || ' and DMT.GAL_CURRENCY_RISK_VIRTUAL_ID is null ';
    else
      vWhereClause  := ' DMT.GAL_CURRENCY_RISK_VIRTUAL_ID is null ';
    end if;
  ...
  begin
    ...
    if (acsFinancialCurrencyID is not null) then
      ...
      DOC_DOCUMENT_FUNCTIONS.changeDocumentCurrRate(aDocumentId, vRateExchange, vBasePrice);
    end if;
   ...
   */
  function GetNewRate(acsFinancialCurrencyID in ACS_FINANCIAL_CURRENCY.ACS_FINANCIAL_CURRENCY_ID%type, aRevaluationDate in date)
    return number
  is
    vReturn             number(1);
    vRateExchange       number;
   ...
   */
  function GetBasePrice(acsFinancialCurrencyID in ACS_FINANCIAL_CURRENCY.ACS_FINANCIAL_CURRENCY_ID%type, aRevaluationDate in date)
    return number
  is
    vReturn             number(1);
    vRateExchange       number;
...
end DOC_DOCUMENT_REVALUATION_FCT;
