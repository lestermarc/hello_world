--------------------------------------------------------
--  DDL for Package Body DOC_EDI_EDEC_ETRANS_V1_1
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "DOC_EDI_EDEC_ETRANS_V1_1" 
is
  ...
  /**
  * function getIncotermsPlace
  * Description
  *   Renvoi le lieu Incoterms d'un envoi
  */
  function getIncotermsPlace(aPackingListID in DOC_PACKING_LIST.DOC_PACKING_LIST_ID%type)
    return varchar2
  is
    ln_cpt       integer;
    lv_incoterms DOC_DOCUMENT.DMT_INCOTERMS_PLACE%type;
  begin
    ln_cpt  := 0;

    for ltplDocument in (select   substr(DMT.DMT_INCOTERMS_PLACE, 1, 20) as DMT_INCOTERMS_PLACE
                             from DOC_PACKING_LIST PAL
                                , DOC_PACKING_PARCEL PAR
                                , DOC_PACKING_PARCEL_POS PAP
                                , DOC_DOCUMENT DMT
                            where PAL.DOC_PACKING_LIST_ID = aPackingListID
                              and PAL.DOC_PACKING_LIST_ID = PAR.DOC_PACKING_LIST_ID
                              and PAR.DOC_PACKING_PARCEL_ID = PAP.DOC_PACKING_PARCEL_ID
                              and PAP.DOC_DOCUMENT_ID = DMT.DOC_DOCUMENT_ID
                              and DMT.DMT_INCOTERMS_PLACE is not null
                         group by DMT.DMT_INCOTERMS_PLACE) loop
      ln_cpt  := ln_cpt + 1;

      if ln_cpt = 1 then
        lv_incoterms  := ltplDocument.DMT_INCOTERMS_PLACE;
      else
        lv_incoterms  := null;
      end if;
    end loop;
  ...
  */
  ...
  begin
    ...
    /* Consignee_Address, facturation */
    begin
      select *
        into tplAddress5
        from DOC_EDEC_ADDRESSES
       where DOC_EDEC_HEADER_ID = aEdecHeaderId
         and C_EDEC_ADDRESS_TYPE = '5';

      vMasterData.Consignee_Address.Line1         := tplAddress5.DEA_ADDRESS_SUPPLEMENT_1;
      vMasterData.Consignee_Address.Line2         := tplAddress5.DEA_ADDRESS_SUPPLEMENT_2;
      vMasterData.Consignee_Address.Country_Code  := GetCountryCode(tplAddress5.PC_CNTRY_ID);
      vMasterData.Consignee_Address.name          := tplAddress5.DEA_NAME;
      vMasterData.Consignee_Address.Place         := tplAddress5.DEA_CITY;
      vMasterData.Consignee_Address.Street        := tplAddress5.DEA_STREET;
      vMasterData.Consignee_Address.Zip_Code      := replace(tplAddress5.DEA_POSTAL_CODE, ' ', '');
    exception
      when no_data_found then
        null;
    end;

    /* Forwarder_Address, transitaire */
    /* Forwarder_Address */
    select *
      into tplAddress3
      from DOC_EDEC_ADDRESSES
    ...
    vMasterData.Forwarder_Address.Zip_Code          := replace(tplAddress3.DEA_POSTAL_CODE, ' ', '');
    ...
    begin
      ...
      vMasterData.Additional_Address_1.Address_Type  := 'DP';
      vMasterData.Additional_Address_1.Line1         := tplAddress1.DEA_ADDRESS_SUPPLEMENT_1;
      vMasterData.Additional_Address_1.Line2         := tplAddress1.DEA_ADDRESS_SUPPLEMENT_2;
      vMasterData.Additional_Address_1.Country_Code  := GetCountryCode(tplAddress1.PC_CNTRY_ID);
      vMasterData.Additional_Address_1.name          := tplAddress1.DEA_NAME;
      vMasterData.Additional_Address_1.Place         := tplAddress1.DEA_CITY;
      vMasterData.Additional_Address_1.Street        := tplAddress1.DEA_STREET;
      vMasterData.Additional_Address_1.Zip_Code      := replace(tplAddress1.DEA_POSTAL_CODE, ' ', '');
    exception
      when no_data_found then
        null;
    end;
    ...
    vMasterData.Torder.Forwarder_Abbrevation        := tplAddress3.DEA_REFERENCE;

    -- DSA - 17.09.2013 : monnaie document
    begin
      select CUR.CURRENCY
        into vMasterData.TOrder.invoice_currency_code
        from DOC_DOCUMENT DMT
           , V_ACS_FINANCIAL_CURRENCY CUR
       where (   DMT.DOC_DOCUMENT_ID = tplHeader.DOC_DOCUMENT_ID
              or DMT.DOC_DOCUMENT_ID in(select DOC_DOCUMENT_ID
    ...
    exception
      when others then
        vMasterData.TOrder.invoice_currency_code  := null;
    end;   -- fin DSA

    if tplHeader.DOC_DOCUMENT_ID is not null then
      select substr(DMT.DMT_INCOTERMS_PLACE, 1, 20)
        into vMasterData.TOrder.Place_of_Incoterm
        from DOC_DOCUMENT DMT
       where DMT.DOC_DOCUMENT_ID = tplHeader.DOC_DOCUMENT_ID;
    elsif tplHeader.DOC_PACKING_LIST_ID is not null then
      vMasterData.TOrder.Place_of_Incoterm  := getIncotermsPlace(tplHeader.DOC_PACKING_LIST_ID);
    end if;
    ...
    -- initialisation des données de ligne Emballage
      /* vTblLineItem.Article */
    vIndex                                          := 1;
    vTblLineItem(vIndex).Article.Order_Number       := vMasterData.TOrder.Order_Number;
    vTblLineItem(vIndex).Article.Packing_Type       := '1';
    vTblLineItem(vIndex).Article.Additional_Pieces  := 1;

    -- initialisation des données de lignes Article
    for tplLineItem in (select   *
                            from DOC_EDEC_POSITION
                           where DOC_EDEC_HEADER_ID = aEdecHeaderId
                        order by DEP_POS_NUMBER asc) loop
      vIndex                                                 := vTblLineItem.count + 1;
      /* vTblLineItem.Article */
      vTblLineItem(vIndex).Article.Order_Number              := vMasterData.TOrder.Order_Number;

      -- Réf du bien
      select max(GOO.GOO_MAJOR_REFERENCE)
        into vTblLineItem(vIndex).Article.Article_Number
        from DOC_EDEC_POSITION_DETAIL DET
           , GCO_GOOD GOO
           , DOC_POSITION POS
       where DET.DOC_EDEC_POSITION_ID = tplLineItem.DOC_EDEC_POSITION_ID
         and DET.DOC_POSITION_ID = POS.DOC_POSITION_ID
         and POS.GCO_GOOD_ID = GOO.GCO_GOOD_ID;

      -- description de l'article
      vTblLineItem(vIndex).Article.Article_description       := tplLineItem.DEP_DESCRIPTION;
      ...
      vTblLineItem(vIndex).Article.Customs_Tariff_Number     := replace(tplLineItem.DEP_COMMODITY_CODE, '.');
      ...
      vTblLineItem(vIndex).Article.Packing_Type              := '2';
      ...
      vTblLineItem(vIndex).Edec.eDec_Customs_Clearence_Type  := tplLineItem.C_EDEC_CUSTOMS_CLEARANCE_TYPE;
    end loop;
  end pInitializeData;
  ...
  */
  ...
  begin
    ...
    if vHeaderNumber is not null then
      ...
      GenerateAddresses(aHeaderID => aHeaderID);
    end if;
  end GenerateHeader;
  ...
  */
  ...
  begin
    ...
    if aDocumentID is not null then
      ...
      if aThirdDeliveryID is not null then
        ...
        select substr(nvl(DMT.DMT_NAME2, PER.PER_NAME), 1, 35)
          ...
          into vAddress.DEA_NAME
             ...
             , vAddress.PAC_ADDRESS_ID
             , vAddress.PAC_THIRD_ID
          from DOC_DOCUMENT DMT
             , PAC_ADDRESS ADR
             , PAC_PERSON PER
         where DMT.DOC_DOCUMENT_ID = aDocumentID
           and DMT.PAC_PAC_ADDRESS_ID = ADR.PAC_ADDRESS_ID(+)
           and ADR.PAC_PERSON_ID = PER.PAC_PERSON_ID(+);
      end if;

      if    aThirdDeliveryID is null
         or vAddress.PAC_ADDRESS_ID is null then
        -- Adresse du partenaire de facturation
        vAddress.DEA_NAME                  := null;
        vAddress.DEA_STREET                := null;
        vAddress.DEA_ADDRESS_SUPPLEMENT_1  := null;
        vAddress.DEA_ADDRESS_SUPPLEMENT_2  := null;
        vAddress.DEA_POSTAL_CODE           := null;
        vAddress.DEA_CITY                  := null;
        vAddress.PC_CNTRY_ID               := null;
        vAddress.PAC_ADDRESS_ID            := null;
        vAddress.PAC_THIRD_ID              := null;

        select substr(nvl(DMT.DMT_NAME3, PER.PER_NAME), 1, 35)
          ...
          from DOC_DOCUMENT DMT
             ...
             , PAC_PERSON PER
         where DMT.DOC_DOCUMENT_ID = aDocumentID
           and DMT.PAC2_PAC_ADDRESS_ID = ADR.PAC_ADDRESS_ID
           and ADR.PAC_PERSON_ID = PER.PAC_PERSON_ID;
      end if;
    -- Source des données = envoi (DOC_PACKING_LIST)
      ...
      if vDelivAddress = 1 then
        select PAL.PAC_PAC_ADDRESS_ID
             , PAL.PAC_THIRD_DELIVERY_ID
             , substr(PAL.PAL_NAME12, 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS12, 1, chr(10) ), 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS12, 2, chr(10) ), 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS12, 3, chr(10) ), 1, 35)
             , substr(PAL.PAL_POSTCODE12, 1, 9)
             , substr(PAL.PAL_TOWN12, 1, 35)
             , PC__PC_CNTRY_ID
          into vAddress.PAC_ADDRESS_ID
             , vAddress.PAC_THIRD_ID
             , vAddress.DEA_NAME
             , vAddress.DEA_STREET
             , vAddress.DEA_ADDRESS_SUPPLEMENT_1
        ...
        -- Adresse du partenaire de facturation
        select PAL.PAC2_PAC_ADDRESS_ID
             , PAL.PAC_THIRD_ACI_ID
             , substr(PAL.PAL_NAME13, 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS13, 1, chr(10) ), 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS13, 2, chr(10) ), 1, 35)
             , substr(extractline(PAL.PAL_ADDRESS13, 3, chr(10) ), 1, 35)
             , substr(PAL.PAL_POSTCODE13, 1, 9)
             , substr(PAL.PAL_TOWN13, 1, 35)
             , PC_2_PC_CNTRY_ID
          into vAddress.PAC_ADDRESS_ID
             , vAddress.PAC_THIRD_ID
             , vAddress.DEA_NAME
  ...
  begin
    ...
    if aDocumentID is not null then
      -- Adresse TRANSPORTEUR
      select substr(SEN.SEN_KEY, 1, 8)
           , nvl(substr(PER.PER_NAME, 1, 35), SEN_KEY)
           ...
           , nvl(ADR.PC_CNTRY_ID, 1)
        into vAddress.DEA_REFERENCE
           , vAddress.DEA_NAME
           , vAddress.DEA_STREET
           , vAddress.DEA_ADDRESS_SUPPLEMENT_1
           , vAddress.DEA_ADDRESS_SUPPLEMENT_2
    ...
    -- Source des données = envoi (DOC_PACKING_LIST)
      ...
      -- Adresse TRANSPORTEUR
      select pal.PAC3_PAC_ADDRESS_ID
           , substr(SEN.SEN_KEY, 1, 8)
           , substr(PAL.PAL_NAME21, 1, 35)
           , substr(extractline(PAL.PAL_ADDRESS21, 1, chr(10) ), 1, 35)
           , substr(extractline(PAL.PAL_ADDRESS21, 2, chr(10) ), 1, 35)
           , substr(extractline(PAL.PAL_ADDRESS21, 3, chr(10) ), 1, 35)
           , substr(PAL.PAL_POSTCODE21, 1, 9)
           , substr(PAL.PAL_TOWN21, 1, 35)
           , PC_3_PC_CNTRY_ID
        into vAddress.PAC_ADDRESS_ID
           , vAddress.DEA_REFERENCE
           , vAddress.DEA_NAME
           , vAddress.DEA_STREET
           , vAddress.DEA_ADDRESS_SUPPLEMENT_1
           , vAddress.DEA_ADDRESS_SUPPLEMENT_2
  ...
  begin
    ...
    if aDocumentID is not null then
      select substr(PER.PER_NAME, 1, 35)
           , substr(extractline(DMT.DMT_ADDRESS3, 1, chr(10) ), 1, 35)
           , substr(extractline(DMT.DMT_ADDRESS3, 2, chr(10) ), 1, 35)
           , substr(extractline(DMT.DMT_ADDRESS3, 3, chr(10) ), 1, 35)
           , substr(DMT.DMT_POSTCODE3, 1, 9)
    ...
    -- Insertion de l'adresse Facturation
    insert into DOC_EDEC_ADDRESSES
         values vAddress;
  end pGenerateAddress5;
  ...
  */
  ...
  begin
    ...
    pGenerateAddress5(aHeaderID, lDocumentID, lThirdDeliveryID, lPackingListID);
  ...
  */
  ...
  begin
    ...
    for tplPos in (select   substr(CUS.CUS_CUSTONS_POSITION, 1, 9) as CUS_CUSTONS_POSITION
                          , CUS.CUS_KEY_TARIFF
                          , nvl(CUS.C_EDEC_PERMIT_OBLIGATION, vCfg_PermitObligation) as C_EDEC_PERMIT_OBLIGATION
                          , nvl(CUS.C_EDEC_COMMERCIAL_GOOD, vCfg_CommercialGood) as C_EDEC_COMMERCIAL_GOOD
                          , CUS.PC_ORIGIN_PC_CNTRY_ID
                          , CUS.C_EDEC_PREFERENCE
                          , CUS.DIC_UNIT_OF_MEASURE_ID
                          , nvl(POS.POS_GROSS_WEIGHT, 0) as POS_GROSS_WEIGHT
                          , case
                              when nvl(POS.POS_NET_WEIGHT, 0) = 0 then nvl(POS.POS_GROSS_WEIGHT, 0)
                              else POS.POS_NET_WEIGHT
                            end as POS_NET_WEIGHT
                          , nvl(POS.POS_FINAL_QUANTITY_SU, 0) * nvl(CUS.CUS_CONVERSION_FACTOR, 1) as DEP_ADDITIONAL_UNIT
                          , nvl(POS.POS_NET_VALUE_INCL_B, 0) as POS_NET_VALUE_INCL_B
                          , POS.DOC_POSITION_ID
                          , POS.POS_LONG_DESCRIPTION as POS_SHORT_DESCRIPTION
      ...
      insert into DOC_EDEC_POSITION
        ...
        select vNewPosID as DOC_EDEC_POSITION_ID
             ...
             , tplPos.POS_SHORT_DESCRIPTION as DEP_DESCRIPTION
             ...
             , tplPos.C_EDEC_CUSTOMS_CLEARANCE_TYPE as C_EDEC_CUSTOMS_CLEARANCE_TYPE
      ...
      insert into DOC_EDEC_POSITION_DETAIL
        ...
        select INIT_ID_SEQ.nextval as DOC_EDEC_POSITION_DETAIL_ID
             ...
             , tplPos.DOC_POSITION_ID as DOC_POSITION_ID
             , sysdate as A_DATECRE
             , PCS.PC_I_LIB_SESSION.GetUserIni as A_IDCRE
          from dual;
